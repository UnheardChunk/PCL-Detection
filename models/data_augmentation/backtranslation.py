...
def backtranslate(texts, batch_size=500):
    back_translated = []
    for i in tqdm(range(0, len(texts), batch_size)):
        batch = texts[i:i+batch_size]
        
        try:
            # Try Google translation first
            translated = GoogleTranslator(source="en", target="japanese").translate_batch(batch)
            back_translated = GoogleTranslator(source="japanese", target="en").translate_batch(batch)
        except Exception as e:
            print(f"Google translation failed for batch {i//batch_size}: {e}. Falling back to MyMemory translator.")
            translated = MyMemoryTranslator(source="en", target="japanese").translate_batch(batch)
            back_translated = MyMemoryTranslator(source="japanese", target="en").translate_batch(batch)
        
        back_translated += GoogleTranslator(source="japanese", target="en").translate_batch(translated)
...
if __name__ == "__main__":
    ...
    translated = augment_dataset(pd.DataFrame(dataset))
    ...
    for i in range(len(texts)):
        ...
        print(f"Backtranslated: {translated_texts[i]}\n")
        print()
